...
PERIODIC_RESOURCE(res_pressure,
                  "title=\"Pressure\";rt=\"pressure\";if=\"sensor\";ct=\"application/senml+json\";obs",
                  res_init,
                  res_get_handler,
...
callback(struct parameter *p)
{
  if( !strncmp(p->name, CONFIG_PERIODIC_PARAM_NAME, strlen(p->name)) ){
    rest_update_resource_interval(&res_pressure, p->value);
    return 0;
  }
  return 1;
}
...
{
#if REST_DELAY_RES_START && APPS_APPCONFIG
  app_config_create_parameter(res_pressure.url, CONFIG_PERIODIC_PARAM_NAME, "0", callback);
#endif
}
...
res_get_handler(void *request, void *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset)
{
  ...
  COAP_BLOCKWISE_TRANSFER(res_pressure);
}
/*
...
res_periodic_handler()
{
  ...
  if(1) {
    ...
    REST.notify_subscribers(&res_pressure);
  }
}
...
{
	COAP_UPDATE_SETTINGS(res_pressure);
}

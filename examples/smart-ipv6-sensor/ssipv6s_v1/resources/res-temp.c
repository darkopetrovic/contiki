...
{
#if REST_DELAY_RES_START && APPS_APPCONFIG
  app_config_create_parameter(res_temperature.url, CONFIG_PERIODIC_PARAM_NAME, "0", callback);
#endif
}
...
res_get_handler(void *request, void *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset)
{
    ...
    sensors_value[0] = sht21_sensor.value( SHT21_SENSE_TEMP );
    SENSORS_DEACTIVATE(sht21_sensor);

    SENSORS_ACTIVATE(tmp100_sensor);
    sensors_value[1] = tmp100_sensor.value( TMP100_SENSE_TEMP );
    SENSORS_DEACTIVATE(tmp100_sensor);
